...
#include "../../INITs_Uitilities/Queue/queue.h"
...
int queue_push_tail_global(Queue *queue, QueueValue data, unsigned char size)
{
	/* Create a location in the heap to store the data in it */
    QueueValue NewData = malloc(size);

    /* copy ur data in the new heap location */
    memcpy(NewData, data, size);

    /* Push the new heap location */
    return  queue_push_tail(queue, NewData);
}
