...
def draw_lines(img, lines, color=[0, 0, 255], thickness=10):
    ...
    distance = abs(right_b - left_b) / math.sqrt(1 + (right_m - left_m) ** 2)

    # # Annotate the image with the distance
    # cv2.putText(img, f"Distance: {distance:.2f} pixels",
    #             (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

    # Check if the car is out of the lane
    if left_x1 < 0 or left_x2 < 0 or right_x1 >= img.shape[1] or right_x2 >= img.shape[1]:
        # Car is out of the lane, display announcement
        cv2.putText(img, "Car is out of lane!", (50, 100),
...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    return line_img,draw_lines(line_img, lines)
...
def annotate_image_array(image_in):
    ...
    line_image, isInLane = hough_lines(masked_edges, rho, theta,
    ...
    return annotated_image,isInLane


def annotate_image(input_file, output_file):
    """ Given input_file image, save annotated image to output_file """
    annotated_image,isInLane = annotate_image_array(mpimg.imread(input_file))
    plt.imsave(output_file, annotated_image)


def annotate_camera():
    """ Annotate video from the camera """
    cap = cv2.VideoCapture(0)  # 0 for the default camera
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        annotated_frame,isInLane = annotate_image_array(frame)
...
def predict_lane_departure(image):
...
if __name__ == "__main__":
    ...
    image = mpimg.imread(r"D:\Faculty\7th Semester\Final_application\data\maxresdefault.jpg")
    annotated_frame,isInLane= predict_lane_departure(image)
    if isInLane:
        print("Car is in lane")
    else:
        print("Car is out of lane")
    cv2.imshow('Annotated Frame', annotated_frame)
    while True:
        if cv2.waitKey(1) & 0xFF == ord('q'):  # Press 'q' to quit
            break
    # # Annotate the test video
    # clip = VideoFileClip(r"D:\Faculty\7th Semester\Final_application\WhatsApp Video 2024-06-13 at 03.01.20_e908bf5c.mp4")
    # annotated_clip = clip.fl_image(annotate_image_array)
    # annotated_clip.write_videofile("test_annotated.mp4", audio=False)
